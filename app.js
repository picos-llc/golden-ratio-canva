...
const guideDescriptions = {
  thirds:
    "<strong>三分割法 (Rule of Thirds)</strong>画面を縦横3等分し、その交点に主題を配置する古典的で安定性の高いレイアウト。主要な被写体や視線を集める要素を交点付近に置くのがお勧めです。",
  golden_spiral:
    "<strong>黄金螺旋 (Golden Spiral)</strong>黄金比に基づいて描かれる対数螺旋。螺旋の収束点（中心）に視線を誘導するように主題を配置します。螺旋の向きを切り替えて、主要な被写体とマッチさせてください。",
  dynamic_symmetry:
    "<strong>動的対称性 (Dynamic Symmetry)</strong>対角線とそこから対角に下ろした垂線による複雑なグリッド。西洋美術や古典絵画（ルネサンス期など）で非常によく使われ、視線の自然な流れや画面の動き（ダイナミズム）を設計できます。",
  golden_triangle:
    "<strong>黄金三角形 (Golden Triangles)</strong>対角線とその他の角から引いた垂線で分割される三角形。動きのある被写体や対角線の方向性を強調し、緊張感とバランスを両立させます。",
};
...
function getClickedHandle(mx, my, cb) {
  ...
  if (Math.abs(mx - cb.x) < tolerance && Math.abs(my - cb.y) < tolerance)
    return "tl";
  if (
    Math.abs(mx - (cb.x + cb.w)) < tolerance &&
    Math.abs(my - cb.y) < tolerance
  )
    return "tr";
  if (
    Math.abs(mx - cb.x) < tolerance &&
    Math.abs(my - (cb.y + cb.h)) < tolerance
  )
    return "bl";
  if (
    Math.abs(mx - (cb.x + cb.w)) < tolerance &&
  ...
  if (currentRatio === "free") {
    if (
      Math.abs(mx - (cb.x + cb.w / 2)) < tolerance &&
      Math.abs(my - cb.y) < tolerance
    )
      return "t";
    if (
    ...
    if (
      Math.abs(mx - cb.x) < tolerance &&
      Math.abs(my - (cb.y + cb.h / 2)) < tolerance
    )
      return "l";
    if (
      Math.abs(mx - (cb.x + cb.w)) < tolerance &&
...
inputSaturation.addEventListener("input", (e) => {
  saturationValue = parseInt(e.target.value, 10);
...
function updateRatiosForOrientation() {
  ...
  btn43.querySelector(".ratio-label").innerText = isPortrait
    ? "3:4 (縦向き標準)"
    : "4:3 (標準画角)";
  btn43.querySelector(".ratio-sub").innerText = isPortrait
    ? "縦向き肖像画・キャンバス"
    : "古典的な油絵キャンバス比";

  const btn32 = document.getElementById("ratio32");
  btn32.dataset.ratio = isPortrait ? "0.667" : "1.5";
  btn32.querySelector(".ratio-label").innerText = isPortrait
    ? "2:3 (縦向き写真)"
    : "3:2 (写真標準)";
  btn32.querySelector(".ratio-sub").innerText = isPortrait
    ? "縦向き構図・ポートレート"
    : "絵画的風景画に最適";

  const btn169 = document.getElementById("ratio169");
  btn169.dataset.ratio = isPortrait ? "0.562" : "1.777";
  btn169.querySelector(".ratio-label").innerText = isPortrait
    ? "9:16 (モバイル)"
    : "16:9 (シネマ)";
  btn169.querySelector(".ratio-sub").innerText = isPortrait
    ? "スマートフォン・縦長動画"
    : "ダイナミックな映画風横長";

  const btnGolden = document.getElementById("ratioGolden");
  btnGolden.dataset.ratio = isPortrait ? "0.618" : "1.618";
  btnGolden.querySelector(".ratio-label").innerText = isPortrait
    ? "1:1.618 (縦黄金比)"
    : "1.618 (黄金長方形)";
  btnGolden.querySelector(".ratio-sub").innerText = isPortrait
    ? "美の調和・縦長キャンバス"
    : "最も調和する美の比率";
...
document.querySelectorAll(".btn-spiral-ctrl").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    document.querySelectorAll(".btn-spiral-ctrl").forEach((b) => {
      b.classList.remove("active");
    });
    e.target.classList.add("active");
    spiralDirection = parseInt(e.target.dataset.dir, 10);
    draw();
  });
});
